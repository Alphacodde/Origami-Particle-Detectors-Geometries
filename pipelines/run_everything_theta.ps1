param(
    ...
    [string]$Mode = "both",
    [double[]]$ThetaValues = @(15, 30, 45, 60, 75),
    [ValidateSet("kresling", "yoshimura", "miura")]
    [string[]]$Structures = @("kresling", "yoshimura", "miura"),
...
$ResultsDir    = Join-Path $ProjectRoot "Results_Theta_Sweep"
$GenMacroDir   = Join-Path $BuildOutDir "macros\_generated"
$ThetaSweepDir      = Join-Path $ProjectRoot "geometry_theta_sweep"
$ThetaSweepBuildDir = Join-Path $BuildOutDir "geometry_theta_sweep"
...
function Move-LatestRoot([string]$tag, [string]$destPrefix) {
    ...
    Say "  -> moved to Results_Theta_Sweep\$(Split-Path $dest -Leaf)"
    return $dest
}
Say "== origamiDet: run_everything_theta (turn-angle sweep) =="
Say "  ProjectRoot   = $ProjectRoot"
Say "  BuildOutDir   = $BuildOutDir"
Say "  ThetaSweepDir = $ThetaSweepDir"
Say "  Mode          = $Mode"
Say "  Theta values  = $($ThetaValues -join ', ') deg"
Say "  Structures    = $($Structures -join ', ')  (no barrel - see script header)"
...
Say "  Facet count (n/cols) and axial density (layers/m_rings/rows) held at each structure's CONFIG default throughout - only turn angle (twist_deg/crease_angle/fold_angle) varies (see sweep_geometry_by_theta.py docstring)"
Say "  Geometry format: GDML (indexed tessellated solids), differentiated per-vertex Kapton shell for all three structures - see this script's own header comment"
...
if (-not $SkipGeometryGen) {
    SayStep "Step 1: generating theta-sweep geometry (geometry_theta_sweep\, GDML)"
    Push-Location $ProjectRoot
    try {
        Say "-- sweep_geometry_by_theta.py --"
        Invoke-Checked "python" @("diff_geom_macros\sweep_geometry_by_theta.py") "sweep_geometry_by_theta.py failed"
    } finally {
        Pop-Location
    }
} else {
...
if (-not (Test-Path $ThetaSweepDir)) {
    Write-Error "$ThetaSweepDir does not exist - run without -SkipGeometryGen at least once first."
    exit 1
}
$thetaSweepSiliconFiles = Get-ChildItem -Path $ThetaSweepDir -Filter "*_silicon.gdml" -File -ErrorAction SilentlyContinue
if (-not $thetaSweepSiliconFiles -or $thetaSweepSiliconFiles.Count -eq 0) {
    Write-Error "$ThetaSweepDir contains no *_silicon.gdml files - either geometry generation did not write where this script expects, or sweep_geometry_by_theta.py is still writing .stl (see this script's own header ASSUMPTION note - it may need updating to import diff_geom_macros\_differentiated_export.py's GDML export path, the same way sweep_geometry_by_N.py was)."
    exit 1
}
Say "  Step 1 OK: $($thetaSweepSiliconFiles.Count) silicon GDML(s) found in $ThetaSweepDir"
SayStep "Step 1b: mesh QA - SKIPPED (prep_stl.py is STL-specific, not yet updated for GDML)"
SayStep "Step 2: syncing geometry_theta_sweep/macros into $BuildOutDir"
$thetaSweepCopySrcCount = (Get-ChildItem -Path $ThetaSweepDir -File -Recurse -ErrorAction SilentlyContinue | Measure-Object).Count
if ($thetaSweepCopySrcCount -eq 0) {
    Write-Error "$ThetaSweepDir is empty - nothing to sync into $BuildOutDir. Re-run Step 1 (do not pass -SkipGeometryGen) before continuing."
    exit 1
}
Copy-Item -Path $ThetaSweepDir -Destination $BuildOutDir -Recurse -Force
Copy-Item -Path (Join-Path $ProjectRoot "macros\*.mac") -Destination (Join-Path $BuildOutDir "macros") -Force
$thetaSweepCopiedSilicon = (Get-ChildItem -Path $ThetaSweepBuildDir -Filter "*_silicon.gdml" -File -ErrorAction SilentlyContinue | Measure-Object).Count
Say "  Step 2 OK: $thetaSweepCopySrcCount file(s) synced from $(Split-Path $ThetaSweepDir -Leaf); $thetaSweepCopiedSilicon silicon GDML(s) now in $ThetaSweepBuildDir"
if (-not $SkipValidation) {
    ...
    Say "Validation PASSED. Moved to Results_Theta_Sweep\$(Split-Path $dest -Leaf)"
} else {
    SayStep "Step 3: Tier-1 validation SKIPPED (-SkipValidation, not recommended)"
}
SayStep "Step 4: theta-sweep"
$allSilicon = Get-ChildItem -Path $ThetaSweepDir -Filter "*_silicon.gdml" -File
$siliconFiles = $allSilicon | Where-Object {
    $base = $_.BaseName -replace "_silicon$", ""
    if ($base -match "^(kresling|yoshimura|miura)_theta([0-9]+(?:\.[0-9]+)?)$") {
        $structureMatch = $Matches[1]
        $thetaMatch = [double]$Matches[2]
        return ($Structures -contains $structureMatch) -and ($ThetaValues -contains $thetaMatch)
    }
    Write-Warning "Unrecognized theta-sweep filename pattern, skipping: $($_.Name)"
    return $false
}
if ($siliconFiles.Count -eq 0) {
    Write-Warning "No *_silicon.gdml files matched -Structures/-ThetaValues in $ThetaSweepDir - nothing to sweep."
}
$nRun = 0
foreach ($si in $siliconFiles) {
    ...
    Say "`n-- $tag --"
    $geomLines = @(
        "/origami/stlFile geometry_theta_sweep/$($si.Name)"
        "/origami/kaptonStlFile geometry_theta_sweep/$kaptonName"
...
SayStep "Done: $nRun run(s) completed, results in Results_Theta_Sweep\"
