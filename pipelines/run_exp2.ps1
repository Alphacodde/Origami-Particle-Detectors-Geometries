...
$ResultsDir   = Join-Path $ProjectRoot "results_exp2"
...
Say "== origamiDet: run_exp2 (multiple scattering, GDML) =="
Say "  ProjectRoot  = $ProjectRoot"
Say "  BuildOutDir  = $BuildOutDir"
Say "  Si / Kapton  = $SiThicknessMM mm / $KaptonThicknessMM mm"
Say "  Momenta      = $($Momenta -join ', ') GeV/c"
Say "  Events/point = $NPerPoint"
Say "  Vertex smear = 0 mm (fixed - this is a momentum scan; see run_exp3.ps1 for the vertex scan)"
Say "  Run tag      = exp2 (see RunAction.hh/.cc MARK 3b - prevents filename collision with run_exp3.ps1's output)"
...
SayStep "Step 4: Experiment 2 - multiple scattering (momentum sweep)"
...
foreach ($si in $siliconFiles) {
    ...
    $tailLines = @(
        ...
        "/run/initialize"
        ""
        "/origami/run/tag exp2"
        "/origami/gun/mode isotropic4pi"
        "/origami/gun/vertexSmearMm 0"
        "/origami/gun/vertexZOffsetMm 0"
    )
    foreach ($p in $Momenta) {
        $macLines = $geomLines + $tailLines + @(
            "/origami/gun/momentumGeV $p"
            "/run/beamOn $NPerPoint"
        )
        $pTag = ("{0:0.00}" -f $p) -replace '\.', 'p'
        $macPath = Join-Path $GenMacroDir "${tag}_exp2_p${pTag}GeV.mac"
        $macLines | Set-Content -Path $macPath
        Say "  p = $p GeV/c ..."
        Push-Location $BuildOutDir
        & ".\origamiDet.exe" "macros\_generated\${tag}_exp2_p${pTag}GeV.mac"
        Pop-Location
        Move-LatestRoot -tag $tag -destPrefix "exp2"
        $nRun++
    }
}
