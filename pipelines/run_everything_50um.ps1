...
$ResultsDir   = Join-Path $ProjectRoot "results_50um"
$GenMacroDir  = Join-Path $BuildOutDir "macros\_generated"
$GeomSrcDir   = Join-Path $ProjectRoot "geom_50um"
...
Say "== origamiDet: run_everything (GDML, 50um silicon variant) =="
Say "  ProjectRoot  = $ProjectRoot"
Say "  BuildOutDir  = $BuildOutDir"
Say "  Mode         = $Mode"
Say "  Si / Kapton  = $SiThicknessMM mm / $KaptonThicknessMM mm  (Si CHANGED from 0.300mm default - see header)"
...
if (-not $SkipGeometryGen) {
    SayStep "Step 1: generating geometry (GDML, $SiThicknessMM mm silicon)"
    $env:ORIGAMIDET_GEOM_OUTPUT_DIR = $GeomSrcDir
    $env:ORIGAMIDET_SI_THICKNESS_MM = "$SiThicknessMM"
    Say "  Generator output dir (ORIGAMIDET_GEOM_OUTPUT_DIR) = $GeomSrcDir"
    Say "  Generator Si thickness (ORIGAMIDET_SI_THICKNESS_MM) = $SiThicknessMM mm"
    ...
    try {
        ...
        Remove-Item Env:\ORIGAMIDET_SI_THICKNESS_MM -ErrorAction SilentlyContinue
    }
    ...
    if (-not $geomSrcFiles -or $geomSrcFiles.Count -eq 0) {
        Write-Error "Step 1 completed but $GeomSrcDir contains no *_silicon.gdml files - geometry generation did not write where this script expects, or is still writing .stl (check diff_geom_macros\_solid_export.py's export_solid() is the version that writes GDML). Check ORIGAMIDET_GEOM_OUTPUT_DIR / ORIGAMIDET_SI_THICKNESS_MM support in the generator scripts (via _geometry_config.py) before continuing."
        exit 1
    }
    Say "  Step 1 OK: $($geomSrcFiles.Count) silicon GDML(s) found in $GeomSrcDir at $SiThicknessMM mm"
} else {
...
SayStep "Step 2: syncing geom_50um/macros into $BuildOutDir"
