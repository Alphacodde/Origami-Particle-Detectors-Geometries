...
Origami Patterns present an unexplored avenue for detector design. These allow a sensor sheet to be tessellated into rigid polygonal facets, interconnected by a kapton substrate shell. This work compares the performance of three origami structures ---Miura-ori~\cite{Miura1985}, Yoshimura~\cite{Yoshimura1955}, and Kresling~\cite{Kresling2008}-- against a control barrel reference of identical dimensions.
...
\begin{table}[!htbp]
\centering
\caption{Geometric parameters of the simulated structures at fixed $R = 40\text{ mm}$, $L = 120\text{ mm}$, $300\ \mu\text{m}$ Si, and $50\ \mu\text{m}$ Kapton.}
...
Yoshimura & 6 rings $\times$ 8 facets & Fold angle $\theta = 30^\circ$ ($\pi/6\text{ rad}$) \\
Kresling & 6 layers $\times$ 6 sides & Twist angle $\theta = 30^\circ$/layer \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Dead-zone fraction ($f_{\text{dead}}$):} The fraction of $(\theta, z)$ phase space bins where the fold hit density falls below the planar baseline ($R(\theta, z) < 1.0$).
\end{itemize}
...
If a particle crosses the material at an angle $\theta_{\mathrm{local}}$, it travels a longer distance through the material. The path length can be approximately written as

\begin{equation}
t_{\mathrm{path}}
=
\frac{t}{\cos\left(\theta_{\mathrm{local}}\right)}.
\end{equation}

These equations are used to check whether the material-budget calculations in the simulation are working correctly.

GEANT4~\cite{Agostinelli2003} is commonly used in particle-physics simulations. It can simulate particle transport, energy loss, scattering, and interactions with detector materials. In this project, GEANT4 is used to compare the different folded detector geometries.
...
Before any results can be read, it is important to understand the sensitivity of barrel baseline based on number of tiles used to form it ($N_{\text{cols}}$). An inscribed $n$-sided polygon of radius $R$ exhibits a geometric sagitta $s = R(1 - \cos(\pi/n))$. Coarse faceting ($N_{\text{cols}} = 24$, $s = 0.342\text{ mm}$) increases the mean local incidence angle by $1.70^\circ$ relative to fine faceting ($N_{\text{cols}} = 141$, $s = 0.0099\text{ mm}$), shifting acceptance by $+0.512$ pp and mean path length by $+1.54\%$ ($z \approx 6.9$, Table~\ref{tab:barrel_sensitivity}). This is why the high-density fine-tiled barrel ($N_{\text{cols}} = 141$) is employed as the strict reference baseline throughout the subsequent differentiated-material evaluations.

\begin{table}[!htbp]
\centering
\caption{Barrel baseline sensitivity to tile faceting ($500{,}000$ primary pions, $R = 40\text{ mm}$).}
...
\midrule
Barrel (coarse, 24 cols) & 418,388 / 500,000 & 0.836776 & $0.0037944 \pm 1.17\times 10^{-6}$ & $27.97^\circ$ \\
Barrel (fine, 141 cols)   & 415,828 / 500,000 & 0.831656 & $0.0037367 \pm 1.14\times 10^{-6}$ & $26.27^\circ$ \\
\bottomrule
\end{tabular}
}
\end{table}

Table~\ref{tab:headline_results} reports the single-run headline results under the differentiated Kapton mesh and $3\text{ mm}$ vertex smear. All three origami geometries demonstrate statistically significant acceptance gains over the fine-tiled barrel, accompanied by increases in radiation-length traversal.
...
Pairwise hypothesis tests across all six geometric combinations (Table~\ref{tab:pairwise_tests}) confirm that Kresling achieves the highest overall acceptance ($+2.362$ pp over barrel, $t = -128.89$, $d = -81.52$), followed by Yoshimura ($+1.304$ pp, $t = -47.22$, $d = -29.86$) and Miura-ori ($+0.493$ pp, $t = -23.44$, $d = -14.82$). In material cost, Yoshimura incurs the largest penalty ($+8.01\%$ over barrel), exceeding Kresling ($+5.69\%$) and Miura-ori ($+1.88\%$).

\begin{table}[!htbp]
\centering
\caption{Pairwise acceptance comparisons across 5-run replications (Welch's two-sample $t$-test, Holm--Bonferroni corrected).}
\label{tab:pairwise_tests}
\smallskip
\resizebox{\textwidth}{!}{
\begin{tabular}{lcccccc}
\toprule
Pairwise comparison & $\Delta A$ (pp) & $t$-statistic & $p$-value & Holm thresh. & Sig.? & Cohen's $d$ \\
\midrule
...
The underlying run-to-run standard deviations ($\text{SD}_A \le 0.05$ pp, Table~\ref{tab:multirun_results}) across five independent $500{,}000$-event runs per geometry are an order of magnitude smaller than the inter-geometry acceptance margins, which is what makes the Welch's $t$-test results above (Table~\ref{tab:pairwise_tests}) decisive rather than borderline.
...
\FloatBarrier
\subsection{Dead-Zone Fractions and Multi-Objective Pareto Frontier}
\label{sec:deadzone_pareto}

Table~\ref{tab:deadzone_multirun} reports the dead-zone fraction ($f_{\text{dead}}$) across five independent runs. The dead-zone ranking is inversely correlated with acceptance: Miura-ori exhibits negligible dead zones ($0.042\%$), whereas Kresling ($1.451\%$) and Yoshimura ($1.861\%$) retain significant under-dense areas. Figure~\ref{fig:deadzone_threshold} illustrates the dead-zone fraction across detection threshold sweeps.

\begin{table}[!htbp]
\centering
\caption{Dead-zone fraction ($f_{\text{dead}}$) across five replication runs per fold geometry.}
...
While the table shows the deadzone data for a specified threshold, the figure below shows the different values for deadzone ratio across a sweep of different threshold values, ranging from 0 to 1. And, in this case, we can see that the order remains the same, until the very end wherein Kresling takes a small advantage. However, at that point, the advantage is negligible and irrelevant.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/fig_deadzone_threshold_sweep.png}
\caption{Dead-zone fraction sensitivity across detection threshold parameters for candidate fold geometries, showing the 3D response surface (left) and 2D mean $\pm$ SEM comparison (right).}
...
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \caption{Acceptance fraction vs.\ dead-zone fraction}
    \label{fig:pareto_a}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
    ...
    \label{fig:pareto_b}
\end{subfigure}
\caption{Pairwise 2D objective trade-off projections for acceptance: (a) acceptance fraction vs.\ dead-zone fraction, and (b) acceptance fraction vs.\ mean $X_0$.}
...
\caption{Mean radiation-length traversal $X_0$ versus dead-zone fraction across candidate fold geometries.}
\label{fig:pareto_c}
\end{figure}
...
\caption{Comprehensive three-panel trade-off matrix pairing acceptance fraction, mean $X_0$, and dead-zone fraction for all geometries.}
...
\begin{itemize}
    ...
    \item \textbf{Miura-ori} minimizes material penalty ($X/X_0 = 4.05 \times 10^{-3}$) and nearly eliminates dead zones ($f_{\text{dead}} = 0.04\%$) with modest acceptance gain ($A = 83.67\%$).
    \item \textbf{Yoshimura} is Pareto-dominated, incurring both the highest material cost ($X/X_0 = 4.30 \times 10^{-3}$) and highest dead-zone fraction ($1.86\%$) for inferior acceptance ($84.48\%$).
\end{itemize}

\FloatBarrier
\subsection{Spatial Structure of Dead Zones and Overlaps}
...
\begin{itemize}
    \item \textbf{Miura-ori (Figure~\ref{fig:map_miura}):} Displays a smooth, quasi-uniform hit-density profile ($R \approx 1$) and continuous material gradient, matching its low dead-zone fraction.
    \item \textbf{Yoshimura (Figure~\ref{fig:map_yoshimura}):} Shows sharp periodic diamond bands where overlap seams ($R > 1.5$) alternate with localized dead-zone valleys ($R < 1$).
...
These differences are small compared with the run-to-run spread measured for the $300\text{ }\mu\text{m}$ simulations in Table~\ref{tab:multirun_results}, where $\mathrm{SD}_A \leq 0.05\text{ pp}$. This suggests that the acceptance difference between the geometries is mainly controlled by the geometry of the facets, seams, and dead zones rather than by the silicon thickness.
...
As pointed out in the fold angle sweep, Yoshimura's nominal fold angle ($30^\circ$) operates in the lower-material regime of its parameter space; increasing $\theta$ yields higher acceptance but causes radiation length to escalate rapidly.
...
The thin-silicon evaluation at $50\text{ }\mu\text{m}$ (Section~\ref{sec:thin_silicon}) reinforces these conclusions across sensor thicknesses. Reducing the silicon thickness from $300\text{ }\mu\text{m}$ to $50\text{ }\mu\text{m}$ decreases overall material traversal by $\sim 4.8\times$ while leaving geometric acceptance gains virtually unchanged ($+2.37$ pp for Kresling, $+1.26$ pp for Yoshimura, and $+0.59$ pp for Miura-ori). When compared against the bent, continuous sensor of ALICE ITS3 at matched physical scope, the flat-faceted tessellations retain a modest silicon-only material penalty ($+0.010$--$0.016$ pp in $X/X_0$) due to non-normal local incidence angles.
...
The quantitative findings reveal that origami folding does not magically circumvent the dead-zone/overlap dilemma; rather, it reshapes and redistributes the trade-off surface.
For acceptance-critical applications where hermetic tracking coverage is paramount, \textbf{Kresling} is the preferred geometry ($+2.36$ pp acceptance gain), provided that tracking algorithms can handle localized material spikes at fold vertices. For precision momentum spectroscopy where Coulomb scattering dominates track errors, \textbf{Miura-ori} provides the optimal balance, delivering a reliable acceptance gain ($+0.49$ pp) with minimal material inflation ($+1.88\%$) and uniform spatial response. \textbf{Yoshimura} is uncompetitive under standard cylindrical envelopes due to its excessive overlap seams.
...
This project compared three cylindrically deployable origami fold patterns --- Miura-ori~\cite{Miura1985}, Yoshimura~\cite{Yoshimura1955}, and Kresling~\cite{Kresling2008} --- against a conventional flat-tile barrel detector of matching radius, using \textsc{Geant4}~\cite{Agostinelli2003} simulations of 5~GeV/$c$ pions to evaluate hit acceptance, radiation-length traversal, and spatial dead-zone/overlap structure for each fully deployed geometry. A dedicated flat-plate model was used to validate the overall \textsc{Geant4} setup.
...
Furthermore, extending the simulation to a $50\text{ }\mu\text{m}$ thin-silicon regime (matching ALICE ITS3) confirms that the geometric acceptance advantages of folding are invariant under sensor thinning, while scaling total material traversal down by $\sim 4.8\times$. While folded planar facets cannot match the silicon-only material minimization of a continuous curved sensor, origami tessellations provide a viable and mathematically rigorous architectural framework for next-generation tracking detectors.

Overall, this study presents the first systematic \textsc{Geant4} radiation-transport comparison of cylindrically deployable origami fold patterns against a planar-tiled detector baseline. Using a physically realistic, differentiated silicon/Kapton mesh, we show that folding consistently improves geometric acceptance at the expense of increased radiation-length traversal. Miura-ori and Kresling define distinct, highly attractive operating points on the Pareto frontier, establishing origami tessellation as a viable and compelling geometric design strategy for future deployable tracking detectors.
\acknowledgments
The author thanks Prof. Biplob Bhattacharjee for his valuable feedback on the initial idea for this work. The author is deeply grateful to his parents for their unwavering support, encouragement, and patience throughout the course of this work, and for making it possible to pursue it in the first place. Claude (Anthropic) was used as an auxiliary tool for debugging and troubleshooting code during the development of the simulation and analysis pipeline.
...
All headline and baseline simulation datasets were generated under the differentiated-mesh \textsc{Geant4} v11.x pipeline utilizing a nominal $3\text{ mm}$ 3D Gaussian vertex smear and $500{,}000$ primary $5\text{ GeV}/c\ \pi^+$ events per configuration at the fully deployed nominal state (fold parameter $= 1.00$). Parameter sweep evaluations systematically varied facet count $N$, fold angle $\theta$, and vertex smear $\sigma$ around these nominal settings. Replication studies (Tables~\ref{tab:multirun_results} and~\ref{tab:deadzone_multirun}) utilize five independent runs with distinct pseudo-random engine seeds.
